...
`{{posts?type=book}}` for a custom post type. A radio group renders a single page of REST
results (10 by default, 100 at most), so use it for short lists and prefer `combobox` when the
list is larger than one page. See [Dynamic Options](dynamic-options).
