	...
	const renderField = () => {
		switch (fieldType) {
			...
			case 'select': {
				...
				const selectOptions = [{label: __('Select...', 'native-custom-fields'), value: null}].concat(resolvedOptions);
			...
			case 'media_library':
				...
				/>);
			case 'token_field': {
				const { allowOnlySuggestions, __experimentalValidateInput: validateInputProp, ...tokenFieldProps } = restElementProps;
